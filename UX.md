...
One case is deliberately kept separate from those two: if the built-in reader itself can't be loaded, every duration read in the app fails before it ever reaches a fallback. That's an incomplete install rather than a misconfigured one, and the prompt says so — naming mutagen and how to restore it, rather than silently offering an ffmpeg reinstall and sending the user round in circles fixing something that was never the problem. It does still offer ffmpeg, though, because ffprobe covers the same formats and so stands in for the built-in reader completely: durations come back even while mutagen stays broken. So that prompt names the cause and offers the workaround in one place, and drops to purely informational once ffprobe is already installed, where there's nothing left to add. The reader is asked for only the four formats the app documents (Ogg Vorbis, MP3, WAV, M4A) for a related reason: mutagen's format sniffing imports two dozen format modules when left unscoped, and in a packaged build one missing module would otherwise take every duration read down with it.
